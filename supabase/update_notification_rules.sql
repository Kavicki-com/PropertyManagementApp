-- update_notification_rules.sql
-- Fixed version: Uses table aliases (n.*) to resolve ambiguity between columns and output variables.

-- 1. DROP old functions to ensure clean slate for logic
...
-- 2. RECREATE check_and_create_rent_due_notifications with new rules
...
BEGIN
  ...
  LOOP
    ...
    IF days_until_due = 5 THEN
      ...
      -- Check if exists today (using alias 'n' to avoid ambiguity)
      ...
      IF existing_notification_id IS NULL THEN
        ...
        RETURNING id, user_id, type, title, body, data
        INTO notification_id, temp_user_id, temp_type, temp_title, temp_body, temp_data;
        
        user_id := temp_user_id; type := temp_type; title := temp_title; body := temp_body; data := temp_data;
        RETURN NEXT;
      END IF;
    END IF;
...
-- 3. NEW FUNCTION: Check User Inactivity
CREATE OR REPLACE FUNCTION check_user_inactivity_notifications()
RETURNS TABLE (
  notification_id UUID,
  user_id UUID,
  type TEXT,
...
BEGIN
  ...
  -- Inactivity
  RETURN QUERY SELECT * FROM check_user_inactivity_notifications();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
