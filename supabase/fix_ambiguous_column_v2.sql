-- fix_ambiguous_column_v2.sql
-- Fixes ALL "column reference is ambiguous" errors by properly aliasing or removing ambiguous RETURNING clauses
  ...
  LOOP
    ...
    IF days_until_due = 5 THEN
      ...
      IF existing_notification_id IS NULL THEN
        -- Insert and capture ONLY the ID in a clear variable
        ...
        -- Assign output variables explicitly
...
DECLARE
  user_map_record RECORD;
  days_inactive INTEGER;
  existing_notification_id UUID;
  new_notification_id UUID;
BEGIN
  -- Iterate users who haven't signed in for > 3 days
  FOR user_map_record IN
    SELECT auth.users.id, auth.users.last_sign_in_at
    FROM auth.users
    WHERE auth.users.last_sign_in_at < (NOW() - INTERVAL '3 days')
  LOOP
    days_inactive := EXTRACT(DAY FROM (NOW() - user_map_record.last_sign_in_at));
    
    -- Check if notification exists in last 3 days (using alias 'n')
    SELECT n.id INTO existing_notification_id
    FROM notifications n
    WHERE n.user_id = user_map_record.id
