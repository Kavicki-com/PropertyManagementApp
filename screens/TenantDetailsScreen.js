...
import {
  ...
  DOCUMENT_TYPES,
} from "../lib/tenantDocumentsService";
...
const TenantDetailsScreen = ({ route, navigation }) => {
    ...
    const fetchDetails = async () => {
      ...
      if (user) {
        ...
        if (!canView) {
          ...
          const requiredPlan =
            currentPlan === "basic" ? "premium" : getRequiredPlan(tenantCount);
      ...
      if (tenantError) {
        ...
        Alert.alert(
          "Erro",
          "Não foi possível buscar os detalhes do inquilino.",
        );
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (financesError) {
              console.error(
                "Erro ao limpar lançamentos financeiros do inquilino:",
                financesError,
              );
              Alert.alert(
                "Erro",
            ...
            if (error) {
              ...
              Alert.alert(
                "Sucesso",
                "Inquilino deletado e registros financeiros de aluguel limpos.",
              );
              navigation.goBack();
            }
          },
        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (contract?.id) {
              ...
              if (contractError) {
                console.error(
                  "Erro ao encerrar contrato ativo:",
                  contractError,
                );
              }
            }
  ...
  const handlePhotoPicker = async (useCamera = false) => {
    ...
    if (permission.status !== "granted") {
      Alert.alert(
        ...
        "Você precisa permitir o acesso para adicionar fotos.",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Erro",
        `Não foi possível abrir ${useCamera ? "a câmera" : "a galeria"}.`,
      );
    }
  };
        ...
        if (tenant.photo_url) {
          try {
            ...
            const oldFilePath = oldUrlParts
              .slice(oldUrlParts.indexOf("tenant-photos") + 1)
              .join("/");
            if (oldFilePath) {
          ...
          onPress: async () => {
            ...
            try {
              ...
              const filePath = urlParts
                .slice(urlParts.indexOf("tenant-photos") + 1)
                .join("/");
              if (filePath) {
                await supabase.storage.from("tenant-photos").remove([filePath]);
              }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.blockedContainer}>
          ...
          <Text style={styles.blockedSubMessage}>
            Você está usando {subscriptionInfo?.tenantCount || 0}{" "}
            {subscriptionInfo?.tenantCount === 1 ? "inquilino" : "inquilinos"}.
        ...
        <UpgradeModal
          ...
          propertyCount={
            subscriptionInfo?.propertyCount ||
            subscriptionInfo?.tenantCount ||
            0
          }
    ...
    if (documentType === "outros" && customName) {
      ...
      return documents.some(
        (doc) =>
          ...
          doc.custom_name &&
          doc.custom_name.toLowerCase().trim() ===
            customName.toLowerCase().trim(),
      );
    } else {
  ...
  const handleDocumentPicker = (documentType = null) => {
    ...
    const typeToUse =
      documentType || documentTypeRef.current || selectedDocumentType;
    ...
    if (!typeToUse) {
      ...
      Alert.alert(
        "Erro",
        "Tipo de documento não selecionado. Tente novamente.",
      );
      return;
    }
  ...
  const handleImageSourceSelection = async (useCamera) => {
    try {
      ...
      if (permission.status !== "granted") {
        Alert.alert(
          "Permissão necessária",
          "Você precisa permitir o acesso para adicionar documentos.",
        );
        setSelectedDocumentType(null);
        documentTypeRef.current = null;
        return;
      ...
      Alert.alert(
        "Erro",
        `Ocorreu um erro ao abrir ${useCamera ? "a câmera" : "a galeria"}. Tente novamente.`,
      );
      setSelectedDocumentType(null);
      documentTypeRef.current = null;
    }
  ...
  const uploadDocument = async (fileData, documentType = null) => {
    ...
    try {
      ...
      if (error) {
        Alert.alert(
          "Erro",
          error.message || "Não foi possível fazer upload do documento.",
        );
      } else {
        Alert.alert("Sucesso", "Documento adicionado com sucesso!");
        // Recarregar lista de documentos
        const { data: updatedDocuments } = await fetchTenantDocuments(
          tenant.id,
        );
      ...
      <ScrollView style={styles.scrollContainer}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={[styles.infoValue, !tenant.cpf && styles.emptyValue]}>
              {tenant.cpf || "Não informado"}
            </Text>
          </View>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>RG</Text>
            <Text style={[styles.infoValue, !tenant.rg && styles.emptyValue]}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text
              style={[
                styles.infoValue,
                !tenant.nationality && styles.emptyValue,
              ]}
            >
              {tenant.nationality || "Não informado"}
            </Text>
          </View>
          <View style={styles.infoRow}>
            ...
            <Text
              style={[
                styles.infoValue,
                !tenant.marital_status && styles.emptyValue,
              ]}
            >
              {tenant.marital_status || "Não informado"}
            </Text>
          </View>
          <View style={styles.infoRow}>
            ...
            <Text
              style={[
                styles.infoValue,
                !tenant.profession && styles.emptyValue,
              ]}
            >
              {tenant.profession || "Não informado"}
            </Text>
          </View>
          <View style={styles.infoRow}>
            ...
            <View style={styles.phoneRow}>
              <Text
                style={[styles.infoValue, !tenant.phone && styles.emptyValue]}
              >
            ...
            <TouchableOpacity activeOpacity={0.8} onPress={handleOpenProperty}>
              <View style={styles.contractPropertyCard}>
                <View style={styles.contractPropertyInfo}>
                  <Text
                    style={styles.contractPropertyAddress}
                    numberOfLines={1}
                    ellipsizeMode="tail"
                  >
                ...
                >
                  ...
                  <Text style={styles.contractPropertyActionText}>
                    Encerrar locação
                  </Text>
                </TouchableOpacity>
              </View>
            ...
            <>
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>
                  {contract.lease_term != null
                    ? `${contract.lease_term} meses`
                    : "N/A"}
                </Text>
              </View>
              <View style={styles.infoRow}>
          ...
          {documentsLoading ? (
            ...
            <View style={styles.documentsEmpty}>
              <MaterialIcons
                ...
                size={48}
                color={
                  theme.isHighContrast ? theme.colors.textSecondary : "#ccc"
                }
              />
              ...
              {DOCUMENT_TYPES.map((type) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      !disabled && handleSelectDocumentType(type.key)
                    }
                    disabled={disabled}
                  >
                ...
                <>
                  ...
                  selectedDocument.mime_type.startsWith("image/") ? (
                    ...
                    <View style={styles.documentPdfContainer}>
                      ...
                      <WebView
                        ...
                        onError={() => {
                          ...
                          Alert.alert(
                            "Erro",
                            "Não foi possível carregar o PDF. Tente abrir no navegador.",
                          );
                        }}
                    ...
                    <View style={styles.documentViewerUnsupported}>
                      ...
                      >
                        <Text style={styles.documentViewerOpenButtonText}>
                          Abrir no navegador
                        </Text>
                      </TouchableOpacity>
                    </View>
