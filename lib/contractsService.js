...
export async function fetchContractsByTenant(tenantId) {
  ...
  const { data, error } = await supabase
    .from('contracts')
    .select('id, tenant_id, property_id, start_date, end_date, lease_term, due_day, rent_amount, deposit, status, created_at')
...
export async function fetchActiveContractByTenant(tenantId) {
  ...
  const { data, error } = await supabase
    .from('contracts')
    .select('id, tenant_id, property_id, start_date, end_date, lease_term, due_day, rent_amount, deposit, status')
    .eq('tenant_id', tenantId)
    .eq('status', 'active')
    .maybeSingle();
...
export async function createContract({
  ...
  const { data, error } = await supabase
    ...
    .select('id, tenant_id, property_id, start_date, end_date, lease_term, due_day, rent_amount, deposit, status, user_id, created_at')
    .single();
  ...
  await Promise.all([
    ...
    removeCache(CACHE_KEYS.PROPERTY_DETAILS(propertyId)),
    removeCache(CACHE_KEYS.TENANT_DETAILS(tenantId)),
...
export async function endContract(contractId) {
  ...
  // Buscar dados do contrato antes de encerrar para invalidar cache correto
  ...
  if (!error && contract) {
    ...
    await Promise.all([
      removeCache(CACHE_KEYS.TENANTS),
      removeCache(CACHE_KEYS.PROPERTIES),
      contract.property_id ? removeCache(CACHE_KEYS.PROPERTY_DETAILS(contract.property_id)) : Promise.resolve(),
      contract.tenant_id ? removeCache(CACHE_KEYS.TENANT_DETAILS(contract.tenant_id)) : Promise.resolve(),
      removeCache(CACHE_KEYS.DASHBOARD),
    ]);
  }
