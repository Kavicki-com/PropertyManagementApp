...
function ensurePurchaseListener() {
  ...
  if (listenerConfigured) {
    console.log('IAP: Listener já estava configurado, re-registrando para garantir...');
    listenerConfigured = false; // Força reconfiguração
  }
...
export async function purchaseSubscription(productId) {
  ...
  ensurePurchaseListener();
  // Aguarda 500ms para dar tempo do listener ser registrado no módulo nativo
          ...
          if (historyCode === InAppPurchases.IAPResponseCode.OK) {
            if (historyResults && historyResults.length > 0) {
              ...
              if (purchase && !purchaseCompleted) {
                // IMPORTANTE: Verifica se a compra é RECENTE (feita nos últimos 60 minutos)
                ...
                // Se a compra é MUITO ANTIGA (mais de 60 minutos), ignora
                if (timeSincePurchase > sixtyMinutesInMs) {
                  console.log(`IAP: ⚠️ Compra muito antiga (${Math.floor(timeSincePurchase / 1000)}s atrás), ignorando...`);
                  return; // Ignora e continua polling
                }

                // Verifica se não passou muito tempo desde o início da compra (timeout de 1 minuto)
                ...
                console.log('IAP: Tempo desde início da compra:', Math.floor(timeSincePurchaseStart / 1000), 'segundos');
      ...
      InAppPurchases.purchaseItemAsync(productId)
        .then(async () => {
          ...
          if (!purchaseCompleted) {
            console.log('IAP: ⏳ Listener não confirmou em 3s, iniciando polling de fallback...');
            pollingInterval = setInterval(pollPurchaseHistory, 3000); // 3 segundos
          } else {
            console.log('IAP: ✅ Compra já foi confirmada pelo listener!');
          }
        })
        .catch((error) => {
...
export async function handlePurchaseSuccess(transaction, userId) {
  try {
    ...
    // Configura expiração baseado no ambiente
    if (__DEV__) {
      // Desenvolvimento: 5 minutos para testes rápidos
      expiresAt.setMinutes(expiresAt.getMinutes() + 5);
      console.log('IAP: ⏰ [DEV] Expirando em 5 MINUTOS (desenvolvimento)');
    } else {
      // Produção: 30 dias (assinatura mensal)
      expiresAt.setDate(expiresAt.getDate() + 30);
      console.log('IAP: ⏰ [PROD] Expirando em 30 DIAS (produção)');
    }
