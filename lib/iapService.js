      ...
      const pollPurchaseHistory = async () => {
        ...
        try {
          ...
          if (historyCode === InAppPurchases.IAPResponseCode.OK && historyResults?.length > 0) {
            ...
            // Busca transação recente (5 min) que corresponda ao produto
...
export async function checkAndSyncSubscriptionStatus(userId, forceAppleCheck = false) {
  ...
  try {
    ...
    // 1. Downgrade Automático Local (Gatilho solicitado)
    // Se o plano não for free, e a data de expiração já passou, downgrade imediato.
    // Isso evita mostrar plano Premium para quem já expirou.
    ...
    // 2. Verificação Remota (Apple Store)
    // Só executa se forceAppleCheck for true (ex: botão Restaurar)
