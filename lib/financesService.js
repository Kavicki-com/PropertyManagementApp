...
export async function fetchTenantBillingSummary(tenant) {
  ...
  // Ordenar pagamentos por data (antigos primeiro) para garantir matching FIFO
  if (financesData) {
    financesData.sort((a, b) => new Date(a.date) - new Date(b.date));
  }
  ...
  const hasPaymentForMonth = (dueDate) => {
    ...
    // Janela de pagamento: a partir de 20 dias antes do vencimento
    // Removemos o limite superior para permitir que pagamentos atrasados paguem contas antigas (FIFO)
    ...
    const matchingPayment = financesData.find((payment) => {
      ...
      // Pagamento deve ser posterior à data de início da janela
      return paymentDate >= paymentWindowStart;
    });
