...
export async function checkSubscriptionStatus(userId) {
  ...
  if (plan === 'basic' || plan === 'premium') {
    ...
    const expiresAt = subscription.subscription_expires_at
      ? parseSupabaseDate(subscription.subscription_expires_at)
      : null;
  ...
  const trialEndsAt = subscription.subscription_trial_ends_at
    ? parseSupabaseDate(subscription.subscription_trial_ends_at)
    : null;
  ...
  const gracePeriodEndsAt = subscription.subscription_grace_period_ends_at
    ? parseSupabaseDate(subscription.subscription_grace_period_ends_at)
    : null;
  ...
  // Se o plano é free, considera ativo exceto se explicitamente expirado/cancelado de forma atípica
  if (plan === 'free') {
    // Para plano free, não exigimos data de expiração
    // Se o status for 'active' ou null (padrão), é considerado ativo
    if (subscription.subscription_status === 'active' || !subscription.subscription_status) {
      return { active: true, reason: 'Plano Gratuito Ativo' };
    }
  }
  ...
  if (subscription.subscription_status !== 'active') {
    // Fallback final: se for free e chegamos aqui, é porque o status é diferente de active/null
    if (plan === 'free') {
      // Mas meu fix anterior força 'active' no banco.
      // Se ainda assim for diferente, respeitamos o status do banco (ex: banned)
      return { active: false, reason: `Status: ${subscription.subscription_status}` };
    }
    return { active: false, reason: `Status: ${subscription.subscription_status}` };
  }

  // Se não tem data de expiração e não está em teste, considera inativo (para não-free)
  if (plan !== 'free') {
    return { active: false, reason: 'Assinatura sem data de expiração válida' };
  }

  return { active: true, reason: 'Plano Gratuito' };
}
